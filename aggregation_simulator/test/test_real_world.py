...
def get_test_network(network_name):
    d = get_test_files_directory()
    network_file = os.path.join(d, "%s/%s.txt" % (network_name, network_name))
    network = Network()
...
class TestRealWorld(unittest.TestCase):
    ...
    def test_with_intel6_singles_only(self):
        ...
        network = get_test_network(test_name)
        ...
        test_directory, sample = make_ready_for_test(test_name,"singles")
    ...
    def test_with_intel6_aggregate(self):
        ...
        test_directory, sample = make_ready_for_test(test_name,"aggregates")
