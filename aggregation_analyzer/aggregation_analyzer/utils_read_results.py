...
from utils import *
from utils_location import get_test_location

def read_one_result_into_dictionary(file_path):
...
def read_results(condition, name, kind, host, timestamp, iteration = None):
    ...
    if iteration is None:
        ...
        for file in files:
            ...
            if p:
                try:
                    i = int(p.group(1))
                    if type(i) is int:
                        result[i] = read_results(condition, name, kind, host, timestamp, iteration=i)
                except ValueError:
        ...
        return read_one_result_into_dictionary(file_path)
